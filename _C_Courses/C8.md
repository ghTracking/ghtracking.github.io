...
$$E = \sum h_{ii} + \sum_{i<j} (ii|jj) - (ij|ij) $$
$$ E = \sum h_{ii} + \frac{1}{2} \sum_{i<j} (ii|jj) - (ij|ij)$$
...
Solving $$f(\mathbf{r_1})\psi_i(\bm{r_1}) = \epsilon_i \psi_i(\bm{r_1})$$. So, we want to solve this. Well, it would work fine for atoms if we solved numerically, but not for molecules (why?). Therefore, we expand the molecular orbitals in terms of atomic orbital basis functions.
...
Where $$\rho$$ is a function of $$\bm{r}$$
