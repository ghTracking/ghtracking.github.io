...
Uses definition of excitation operator, whereby all corrections (single, double, triple, etc. up to a given type) are present to infinite order. The couple cluster wave function is written in terms of an exponential. Clusters may be connected (e.g. T$_1$ or disconnected T$_1^2$.).

Instead of using a variational principle, the coupled cluster Schroedinger equation is project onto the reference wavefunction. 
